...
- **Monthly P&L still has an unresolved wide-range report failure:** The earlier DB-layer RPC optimization removed the worst table-scan bottleneck, but the report request for `2025-01-01` through `2026-02-01` can still return `500` from the backend after the latest Render deploy, and the overall P&L page remains too slow. The next Monthly P&L tranche should focus on tracing that exact range and moving imports to an async/background path.
