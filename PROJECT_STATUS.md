...
## 2026-03-16 (ET)
- **Reporting surface separation cleanup landed:** Clarified the `/reports` UX so WBR and Monthly P&L are presented as sibling reporting products rather than one surface inheriting the other. The shared reports hub now explains both surfaces, client marketplace cards show WBR and Monthly P&L readiness separately, the reports header exposes a marketplace-level WBR/P&L switcher, and the Monthly P&L page now explicitly frames itself as a standalone finance report.
- **Monthly P&L provenance surfaced in UI:** Added active-import provenance to the Monthly P&L screen so operators can see the active source filename, import timestamps, import ID, and covered months for the report period currently in view. Added lightweight source-drift guidance in the upload area to clarify that Amazon download dates can shift settlement coverage.
