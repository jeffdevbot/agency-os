...
- **The Whoosh US month-long `/ngram-2` Step 4 failure was traced to workbook generation after AI, not the expensive AI pass itself:** Two full pure-model month runs for Whoosh US actually completed and persisted successfully in `ngram_ai_preview_runs`, but the frontend was redundantly reposting a giant AI payload into the workbook-generation call even though the backend could already rebuild from the saved run. Step 4 now sends only `preview_run_id` plus the needed request metadata into workbook generation.
- **`/ngram-2` now has saved-run recovery so analysts do not have to repay for AI when workbook generation fails after persistence:** The page now surfaces recent saved runs for the current user/profile/date selection and can rebuild the workbook directly from a persisted `preview_run_id` without rerunning the AI stage.
- **Recent identical full `/ngram-2` requests are now reused instead of rerunning AI immediately:** The preview route now reuses a recent identical saved full run for a short window, which reduces accidental duplicate GPT-5.4 spend when a user retries the same full request after a downstream workbook failure.
- **Brand / mix / defensive campaigns are no longer excluded from AI and now use explicit brand-portfolio handling:** `/ngram-2` now loads client/brand names from Command Center context, includes mixed branded defensive campaigns in the AI path, and tells the model to treat those lanes as `brand_portfolio` scope rather than as a single-SKU gate. This prevents terms from being negated only because they point at a sibling in-brand family under the same client portfolio.
- **`/ngram-2` now trims `KEEP` rationale by default to reduce output-token spend:** The prompt now asks the model to return `null` rationale for most `KEEP` rows, while local validation also nulls any stray `KEEP` rationale before persistence. `NEGATE` and `REVIEW` rationale remain intact for analyst usefulness.
...
- **The N-Gram handoff/docs were refreshed again after the real Whoosh month run investigation:** The current docs now describe saved-run recovery, recent-run reuse, brand-portfolio campaign handling, and sparse `KEEP` rationale as the current shipped state instead of leaving the old `Invalid confidence` issue as the main restart target.
...
- **`/ngram-2` Step 3/4 now use OpenAI Structured Outputs instead of prompt-only JSON for campaign evaluation:** The GPT-5.4 N-Gram campaign call now sends a strict `json_schema` `response_format` through Chat Completions, so enum-heavy fields like `match_confidence` and term `confidence` are constrained by the API contract rather than only by prompt instructions; the campaign evaluator still keeps local validation/retry as a backup.
...
- **`/ngram-2` Step 3 now passes explicit marketplace context into the model payload:** The preview route now includes `marketplace_code` alongside campaign/theme/catalog/search-term data so the model does not have to infer CA-vs-US language rules from campaign naming alone.
- **`/ngram-2` preview now has a dedicated model lane with GPT-5 request-shape handling centralized in the shared adapter:** Step 3 no longer has to inherit the generic frontend OpenAI primary model; the preview route now reads `OPENAI_MODEL_NGRAM` explicitly, and GPT-5-family requests now rely on the shared adapter’s no-temperature request shape instead of route-level overrides.
...
- **Current next-session product target:** The critical path is now a full 7-day Whoosh US analyst-reviewed manual-vs-AI workbook comparison. Treat the current `/ngram-2` SP workflow as shipped enough for quality evaluation, and only reopen prompt/model/plumbing work if that comparison exposes a real gap.
