...
- **Current next-session product target:** The trusted preview surface is now wired into workbook prefill and the first override-capture layer is live; the next useful work should focus on `/ngram-2` reviewed-workbook upload UX and analyzing the captured override dataset rather than reopening catalog/model plumbing that is now working.
