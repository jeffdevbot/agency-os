...
- **The AI-prefilled workbook now carries compact review context without breaking the familiar N-Gram layout:** Search-term tables keep their existing structure and now append `AI Recommendation`, `AI Confidence`, and `AI Reason` columns, while summary metadata records the linked preview run id / model / threshold and the scratchpad area shifts one column right to preserve workbook compatibility.
