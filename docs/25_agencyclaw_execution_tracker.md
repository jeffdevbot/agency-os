...
- N-gram special skill cleanup: removed `ngram_research` from `tool_registry`, policy gate mutation set, planner handler map, and orchestrator dispatch. N-gram requests now route through normal `clickup_task_create` + C10C SOP grounding path (no dedicated N-gram execution skill).
