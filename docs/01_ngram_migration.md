...
origins = [
    "http://localhost:3000",
    "[https://tools.ecomlabs.ca](https://tools.ecomlabs.ca)",
]
app.add_middleware(
    CORSMiddleware,
...
const handleUpload = async (file: File) => {
  ...
  const response = await fetch("[https://api-core.ecomlabs.ca/ngram/process](https://api-core.ecomlabs.ca/ngram/process)", {
    method: "POST",
    headers: {
...
Set these Env Vars in the backend-core service:
...
Set these Env Vars in the frontend-web service:
