...
- **Status:** Backend pool CRUD/upload APIs delivered (Nov 20, 2025) with case-insensitive merge/dedupe, CSV parsing, state reset on upload, and validation (min 5, max 5000). Frontend upload UI delivered (Nov 20, 2025) with scope-aware tabs, CSV/paste/manual inputs, drag/drop, template download, dedupe/validation, and optimistic hook + tests.
