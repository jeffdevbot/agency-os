...
2. Raw rows are normalized into a canonical `pnl_ledger_entries` table.
3. Ledger entries are mapped to `pnl categories`.
4. The report UI sums categories into the displayed P&L rows.
...
8. `raw_type`
9. `raw_description`
10. `raw_payload jsonb`
11. timestamps

### 4. `monthly_pnl_ledger_entries`
...
2. `profile_id`
3. `import_id`
4. `entry_month` (`date`, normalized to first day of month)
5. `posted_at`
6. `order_id`
7. `sku`
8. `source_type`
...
13. `amount`
14. `currency_code`
15. `is_mapped boolean`
16. `mapping_rule_id nullable`
17. `raw_payload jsonb`
18. timestamps
...
2. `match_field = type|description`
3. `match_operator = exact_pair`
4. `match_value = Service Fee|Cost of Advertising`
5. `target_bucket = advertising`

### 6. `monthly_pnl_cogs_monthly`
...
6. Parse raw rows into `monthly_pnl_raw_rows`
7. Group rows by `entry_month` using posted/released financial date
8. Expand rows into normalized `monthly_pnl_ledger_entries`
9. Apply mapping rules
10. Flag unmapped entries
11. Recompute monthly aggregates
12. Store import results by month so the UI can show which months were loaded from one file
...
The report service should always resolve to the latest active month-level source for a given profile/month/source class.
...
5. `Posted date vs release date`
   - choose one canonical month basis and document it
6. `Source drift`
