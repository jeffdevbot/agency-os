...
const PROMPT_VERSION = "scribe_topics_v1";
...
const buildPrompt = (skuData: SkuData): string => {
...
6) Each topic must have:
   ...
   - a 1-sentence DESCRIPTION explaining why that angle matters
...
export const generateTopicsForSku = async (skuData: SkuData): Promise<TopicsGenerationResult> => {
  const prompt = buildPrompt(skuData);
