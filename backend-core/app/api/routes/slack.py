        ...
        async def _execute_planner_delegate_for_agent_loop(
            *,
            ...
            try:
                    ...
                    try:
                        ...
                        if isinstance(plan, dict):
                            await asyncio.to_thread(
                                ...
                                child_run_id,
                                "delegate_planner",
                                {"turn": turn, "plan_intent": str(plan.get("intent") or "unknown"), "plan": plan},
                            )
                        await asyncio.to_thread(
                        ...
                        _logger.warning("Planner child-run iteration logging failed", exc_info=True)
                ...
                for turn in range(planner_max_turns):
                    ...
                    if plan_fingerprint in seen_plan_fingerprints:
                        ...
                        if all_mutation_proposals:
                            summary += f" Deferred {len(all_mutation_proposals)} mutation proposal(s)."
                ...
                if all_mutation_proposals:
                    partial_summary += f" Deferred {len(all_mutation_proposals)} mutation proposal(s)."
                if all_unsupported_steps:
