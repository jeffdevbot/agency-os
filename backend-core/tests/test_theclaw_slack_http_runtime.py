...
from app.services.theclaw.slack_http_runtime import (
...
async def test_slack_events_runtime_requires_signing_secret(monkeypatch):
    monkeypatch.setattr(
        "app.services.theclaw.slack_http_runtime.get_slack_signing_secret",
        lambda: "",
    )
...
async def test_slack_events_runtime_retry_is_noop(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.theclaw.slack_http_runtime.verify_request_or_401",
        lambda **kwargs: None,
    )
...
async def test_slack_events_runtime_url_verification(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.theclaw.slack_http_runtime.parse_json_payload",
...
async def test_slack_interactions_runtime_schedules_interaction(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.theclaw.slack_http_runtime.parse_interaction_payload",
