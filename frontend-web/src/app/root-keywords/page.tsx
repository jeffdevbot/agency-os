...
export default function RootKeywordsPage() {
  ...
  const startUpload = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/root/process`, {
        method: "POST",
        headers: {
      ...
      a.href = downloadUrl;
      a.download = `${selectedFile.name.replace(/\.[^.]+$/, "")}_root_keywords.xlsx`;
      ...
      <header className="border-b border-slate-200 bg-white px-6 py-4 shadow-sm">
        <div className="mx-auto flex max-w-6xl items-baseline gap-3">
          ...
          <p className="text-sm text-slate-500">
            Upload your Campaign Report to analyze hierarchical performance by profile, portfolio, and campaign structure.
          </p>
        </div>
      </header>
          ...
          <div className="rounded-3xl bg-white/95 p-8 shadow-[0_30px_80px_rgba(10,59,130,0.15)]">
            <div>
              <h2 className="text-lg font-semibold text-[#0f172a]">
                Generate Root Keyword Analysis
              </h2>
              <p className="text-sm text-[#4c576f]">
                Upload your Campaign Report to generate a hierarchical workbook with 4 weeks of metrics grouped by Profile → Portfolio → Ad Type → Targeting → Sub-Type → Variant.
              </p>
            </div>
            <div
            ...
            >
              <p className="text-base font-semibold text-[#0f172a]">
                Drag & drop your Campaign Report
              </p>
