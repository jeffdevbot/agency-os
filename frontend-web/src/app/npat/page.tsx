...
export default function NpatPage() {
  ...
  const startUpload = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/npat/process`, {
        method: "POST",
        headers: {
      ...
      a.href = downloadUrl;
      a.download = `${selectedFile.name.replace(/\.[^.]+$/, "")}_npat.xlsx`;
  ...
  const startCollect = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/npat/collect`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
    ...
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-[#eaf2ff] via-[#dce8ff] to-[#cddcf8]">
      <header className="border-b border-slate-200 bg-white px-6 py-4 shadow-sm">
        <div className="mx-auto flex max-w-6xl items-baseline gap-3">
          <h1 className="text-xl font-bold tracking-tight text-slate-900">N-PAT PROCESSOR</h1>
          <p className="text-sm text-slate-500">
            Upload your Search Term Report and identify ASINs for negative targeting.
          </p>
        </div>
      </header>
        ...
        <div className="w-full max-w-5xl space-y-10">
          <div className="rounded-3xl bg-white/95 p-8 shadow-[0_30px_80px_rgba(10,59,130,0.15)]">
            <div>
              ...
              <h2 className="mt-1 text-lg font-semibold text-[#0f172a]">Generate the N-PAT workbook</h2>
              <p className="text-sm text-[#4c576f]">
                Upload your Search Term Report to analyze ASINs with Helium10 enrichment zones for product targeting decisions.
              </p>
            </div>
            <div
          ...
          <div className="rounded-3xl bg-white/95 p-8 shadow-[0_30px_80px_rgba(10,59,130,0.15)]">
            <div>
              ...
              <p className="text-sm text-[#4c576f]">
                After enriching ASINs with Helium10 data and marking NE (Negative Exact), upload the completed workbook to export your negative targeting summary.
              </p>
            </div>
