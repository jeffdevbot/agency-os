...
import type { SponsoredBrandsLandingPagesView } from "../../types";
import { formatCurrency, formatNumber, formatPercent } from "../../utils/format";

interface SponsoredBrandsLandingPagesViewProps {
  data: SponsoredBrandsLandingPagesView;
  currency: string;
}
...
export function SponsoredBrandsLandingPagesView({
  data,
  currency,
}: SponsoredBrandsLandingPagesViewProps) {
  const landingPages = data?.landing_pages ?? [];

  if (!landingPages || landingPages.length === 0) {
    return (
      <div className="flex h-full items-center justify-center text-slate-400">
        No Sponsored Brands landing page data available.
      </div>
    );
  }

  const totalSpend = landingPages.reduce((acc, curr) => acc + curr.spend, 0);
  const pieData = landingPages.map((row) => ({
    name: row.landing_page_type,
    value: row.spend,
          ...
          <div className="xl:col-span-7 overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-slate-500 uppercase text-xs font-semibold border-b border-slate-200">
                <tr>
                  <th className="px-6 py-3 font-medium">Landing Page</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {landingPages.map((row, index) => (
                  <tr
                    key={`${row.landing_page_type}-${index}`}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right text-slate-600">
                      {formatPercent(row.spend_percent)}
                    </td>
                    <td className="px-6 py-4 text-right text-slate-900">
                      {formatCurrency(row.sales, currency)}
                    </td>
                    <td className="px-6 py-4 text-right text-slate-600 hidden xl:table-cell">
