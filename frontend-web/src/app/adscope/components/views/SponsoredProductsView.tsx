"use client";

import {
    PieChart,
    Pie,
    Cell,
    Tooltip,
    ResponsiveContainer,
    Legend
} from "recharts";
import type { SponsoredProductsView as SPViewData } from "../../types";
import { formatCurrency, formatNumber, formatPercent } from "../../utils/format";

interface SponsoredProductsViewProps {
    data: SPViewData;
    currency: string;
}

const TARGETING_COLORS = ["#0077cc", "#f59e0b"]; // Blue for Manual, Amber for Auto
const MATCH_TYPE_COLORS = [
    "#0077cc", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6",
    "#ec4899", "#06b6d4", "#84cc16", "#f97316", "#6366f1",
    "#14b8a6", "#a855f7"
];

export function SponsoredProductsView({ data, currency }: SponsoredProductsViewProps) {
    if (!data) {
        return (
            <div className="flex h-full items-center justify-center text-slate-400">
                No Sponsored Products data available.
            </div>
        );
    }

    const { targeting_breakdown, match_types } = data;

    // Prepare pie data for targeting (Auto vs Manual)
    const totalTargetingSpend = targeting_breakdown.reduce((acc, curr) => acc + curr.spend, 0);
    const targetingPieData = targeting_breakdown.map(item => ({
        name: item.targeting_type,
        value: item.spend,
        percent: totalTargetingSpend > 0 ? (item.spend / totalTargetingSpend) * 100 : 0
    }));

    // Prepare pie data for match types
    const totalMatchSpend = match_types.reduce((acc, curr) => acc + curr.spend, 0);
    const matchTypePieData = match_types.map(item => ({
        name: item.match_type,
        value: item.spend,
        percent: totalMatchSpend > 0 ? (item.spend / totalMatchSpend) * 100 : 0
    }));

    return (
        <div className="space-y-6 p-6">
            {/* Section 1: Auto vs Manual */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                    <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wide">Targeting Type (Auto vs Manual)</h3>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-10 gap-0">
                    {/* Pie Chart - 30% */}
                    <div className="lg:col-span-3 p-6 border-b lg:border-b-0 lg:border-r border-slate-100">
                        <div className="h-[220px]">
                            {targetingPieData.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie
                                            data={targetingPieData}
                                            cx="50%"
                                            cy="50%"
                                            innerRadius={50}
                                            outerRadius={70}
                                            paddingAngle={5}
                                            dataKey="value"
                                        >
                                            {targetingPieData.map((entry, index) => (
                                                <Cell key={`cell-${index}`} fill={TARGETING_COLORS[index % TARGETING_COLORS.length]} />
                                            ))}
                                        </Pie>
                                        <Tooltip
                                            formatter={(_value, _name, props) => {
                                                const pct = (props?.payload as { percent?: number })?.percent ?? 0;
                                                return `${pct.toFixed(1)}%`;
                                            }}
                                            contentStyle={{ backgroundColor: "#ffffff", borderColor: "#e2e8f0", color: "#0f172a", borderRadius: "8px", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.1)" }}
                                            itemStyle={{ color: "#0f172a" }}
                                        />
                                        <Legend iconType="circle" />
                                    </PieChart>
                                </ResponsiveContainer>
                            ) : (
                                <div className="flex h-full items-center justify-center text-slate-400 text-sm">
                                    No targeting data
                                </div>
                            )}
                        </div>
                    </div>
                    {/* Table - 70% */}
                    <div className="lg:col-span-7 overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-slate-50 text-slate-500 uppercase text-xs font-semibold border-b border-slate-200">
                                <tr>
                                    <th className="px-6 py-3 font-medium">Type</th>
                                    <th className="px-6 py-3 text-right font-medium"># Campaigns</th>
                                    <th className="px-6 py-3 text-right font-medium">Spend</th>
                                    <th className="px-6 py-3 text-right font-medium">Sales</th>
                                    <th className="px-6 py-3 text-right font-medium">CPC</th>
                                    <th className="px-6 py-3 text-right font-medium">CTR</th>
                                    <th className="px-6 py-3 text-right font-medium">CVR</th>
                                    <th className="px-6 py-3 text-right font-medium">ACoS</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {targeting_breakdown.length > 0 ? (
                                    targeting_breakdown.map((row) => (
                                        <tr key={row.targeting_type} className="hover:bg-slate-50 transition-colors">
                                            <td className="px-6 py-4 font-medium text-slate-900">{row.targeting_type}</td>
                                            <td className="px-6 py-4 text-right text-slate-600">{formatNumber(row.campaigns)}</td>
                                            <td className="px-6 py-4 text-right text-slate-900">{formatCurrency(row.spend, currency)}</td>
                                            <td className="px-6 py-4 text-right text-slate-900">{formatCurrency(row.sales, currency)}</td>
                                            <td className="px-6 py-4 text-right text-slate-600">{formatCurrency(row.cpc, currency)}</td>
                                            <td className="px-6 py-4 text-right text-slate-600">{formatPercent(row.ctr)}</td>
                                            <td className="px-6 py-4 text-right text-slate-600">{formatPercent(row.cvr)}</td>
                                            <td className={`px-6 py-4 text-right font-semibold ${row.acos > 1 ? "text-red-600" : row.acos > 0.4 ? "text-amber-600" : "text-emerald-600"}`}>
                                                {formatPercent(row.acos)}
                                            </td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan={8} className="px-6 py-8 text-center text-slate-400">
                                            No targeting data available
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>

            {/* Section 2: Match Types */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                    <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wide">Match Types</h3>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-10 gap-0">
                    {/* Pie Chart - 30% */}
                    <div className="lg:col-span-3 p-6 border-b lg:border-b-0 lg:border-r border-slate-100">
                        <div className="h-[320px]">
                            {matchTypePieData.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie
                                            data={matchTypePieData}
                                            cx="50%"
                                            cy="50%"
                                            innerRadius={50}
                                            outerRadius={70}
                                            paddingAngle={2}
                                            dataKey="value"
                                        >
                                            {matchTypePieData.map((entry, index) => (
                                                <Cell key={`cell-${index}`} fill={MATCH_TYPE_COLORS[index % MATCH_TYPE_COLORS.length]} />
                                            ))}
                                        </Pie>
                                        <Tooltip
                                            formatter={(_value, _name, props) => {
                                                const pct = (props?.payload as { percent?: number })?.percent ?? 0;
                                                return `${pct.toFixed(1)}%`;
                                            }}
                                            contentStyle={{ backgroundColor: "#ffffff", borderColor: "#e2e8f0", color: "#0f172a", borderRadius: "8px", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.1)" }}
                                            itemStyle={{ color: "#0f172a" }}
                                        />
                                        <Legend
                                            iconType="circle"
                                            layout="vertical"
                                            align="left"
                                            verticalAlign="middle"
                                            wrapperStyle={{ fontSize: "11px", paddingLeft: "10px" }}
                                        />
                                    </PieChart>
                                </ResponsiveContainer>
                            ) : (
                                <div className="flex h-full items-center justify-center text-slate-400 text-sm">
                                    No match type data
                                </div>
                            )}
                        </div>
                    </div>
                    {/* Table - 70% */}
                    <div className="lg:col-span-7 overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-slate-50 text-slate-500 uppercase text-xs font-semibold border-b border-slate-200">
                                <tr>
                                    <th className="px-6 py-3 font-medium">Match Type</th>
                                    <th className="px-6 py-3 text-right font-medium"># Targets</th>
                                    <th className="px-6 py-3 text-right font-medium">Spend</th>
                                    <th className="px-6 py-3 text-right font-medium">Sales</th>
                                    <th className="px-6 py-3 text-right font-medium">CPC</th>
                                    <th className="px-6 py-3 text-right font-medium">CTR</th>
                                    <th className="px-6 py-3 text-right font-medium">CVR</th>
                                    <th className="px-6 py-3 text-right font-medium">ACoS</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {match_types.length > 0 ? (
                                    match_types.map((row, index) => (
                                        <tr key={row.match_type} className="hover:bg-slate-50 transition-colors">
                                            <td className="px-6 py-4 font-medium text-slate-900 flex items-center gap-2">
                                                <span
                                                    className="w-3 h-3 rounded-full"
                                                    style={{ backgroundColor: MATCH_TYPE_COLORS[index % MATCH_TYPE_COLORS.length] }}
                                                />
                                                {row.match_type}
                                            </td>
                                            <td className="px-6 py-4 text-right text-slate-600">{formatNumber(row.target_count)}</td>
                                            <td className="px-6 py-4 text-right text-slate-900">{formatCurrency(row.spend, currency)}</td>
                                            <td className="px-6 py-4 text-right text-slate-900">{formatCurrency(row.sales, currency)}</td>
                                            <td className="px-6 py-4 text-right text-slate-600">{formatCurrency(row.cpc, currency)}</td>
                                            <td className="px-6 py-4 text-right text-slate-600">{formatPercent(row.ctr)}</td>
                                            <td className="px-6 py-4 text-right text-slate-600">{formatPercent(row.cvr)}</td>
                                            <td className={`px-6 py-4 text-right font-semibold ${row.acos > 1 ? "text-red-600" : row.acos > 0.4 ? "text-amber-600" : "text-emerald-600"}`}>
                                                {formatPercent(row.acos)}
                                            </td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan={8} className="px-6 py-8 text-center text-slate-400">
                                            No match type data available
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    );
}
