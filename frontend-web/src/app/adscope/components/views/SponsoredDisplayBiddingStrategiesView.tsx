...
import type { SponsoredDisplayBiddingStrategiesView } from "../../types";
import { formatCurrency, formatNumber, formatPercent } from "../../utils/format";

interface SponsoredDisplayBiddingStrategiesViewProps {
  data: SponsoredDisplayBiddingStrategiesView;
  currency: string;
}
...
export function SponsoredDisplayBiddingStrategiesView({
  data,
  currency,
}: SponsoredDisplayBiddingStrategiesViewProps) {
  const strategies = data?.strategies ?? [];

  if (!strategies || strategies.length === 0) {
    return (
      <div className="flex h-full items-center justify-center text-slate-400">
        No Sponsored Display bidding strategy data available.
      </div>
    );
  }

  const totalSpend = strategies.reduce((acc, curr) => acc + curr.spend, 0);
  const pieData = strategies.map((row) => ({
    name: row.strategy,
    value: row.spend,
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-slate-500 uppercase text-xs font-semibold border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right font-medium">Page Visits</th>
                  <th className="px-6 py-3 text-right font-medium">Conversions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {strategies.map((row, index) => (
                  <tr key={`${row.strategy}-${index}`} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right text-slate-600 font-mono">{formatNumber(row.reach)}</td>
                    <td className="px-6 py-4 text-right text-slate-600 font-mono">{formatNumber(row.page_visits)}</td>
                    <td className="px-6 py-4 text-right text-slate-600 font-mono">{formatNumber(row.conversions)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
