...
describe("POST /projects/:id/unapprove-copy", () => {
  ...
  it("rejects if project is not stage_c_approved", async () => {
    ...
    expect(json.error.message).toContain("Cannot unapprove copy");
  });
  ...
  it("unapproves stage C (back to stage_b_approved)", async () => {
    supabaseMock.supabase.auth.getSession.mockResolvedValue({ data: { session: { user: { id: "user-1" } } } });
    supabaseMock.getBuilder("scribe_projects").__pushResponse({
      data: { id: "proj-1", status: "stage_c_approved", created_by: "user-1" },
      error: null,
    });
    supabaseMock.getBuilder("scribe_projects").__pushResponse({
      data: { id: "proj-1", name: "Proj", status: "stage_b_approved", updated_at: "2025-11-28T00:00:00Z" },
      error: null,
    });
