...
export async function POST(
  ...
  // Validate that each SKU has exactly 5 approved topics
  for (const sku of skus) {
    ...
    if ((count ?? 0) !== 5) {
      return NextResponse.json(
        { error: { code: "validation_error", message: `All SKUs must have exactly 5 approved topics` } },
        { status: 400 },
      );
    }
  }

  // Update project status to stage_b_approved
  const { data, error } = await supabase
    .from("scribe_projects")
    .update({ status: "stage_b_approved", updated_at: new Date().toISOString() })
    .eq("id", projectId)
    .eq("created_by", session.user.id)
  ...
  if (error || !data) {
    return NextResponse.json(
      { error: { code: "server_error", message: error?.message ?? "Unable to approve Stage B" } },
      { status: 500 },
    );
  }
