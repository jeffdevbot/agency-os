...
export async function POST(
  ...
  if (!isStatus(targetStatus) || targetStatus === "archived") {
    return NextResponse.json(
      { error: { code: "validation_error", message: "invalid restore status" } },
      { status: 400 },
    );
  }
  ...
  if (existing.status !== "archived") {
    return NextResponse.json(
      { error: { code: "conflict", message: "Project is not archived" } },
      { status: 409 },
    );
  }
  ...
  if (error || !data) {
    return NextResponse.json(
      { error: { code: "server_error", message: error?.message ?? "Unable to restore project" } },
      { status: 500 },
    );
  }
