...
export async function POST(
  ...
  if (project.status !== "stage_b_approved") {
    return NextResponse.json(
      {
        error: {
          code: "conflict",
          message: "Cannot unapprove topics unless project is stage_b_approved",
        },
      },
      { status: 409 },
    );
  ...
  // Revert status back to stage_a_approved
  ...
  if (error || !data) {
    return NextResponse.json(
      { error: { code: "server_error", message: error?.message ?? "Unable to unapprove Stage B" } },
      { status: 500 },
    );
  }
