...
The private Ecomlabs Tools remote MCP connector is live for shared WBR +
Monthly P&L workflows in Claude, and the next rollout step is moving from the
Jeff-only pilot to Claude Team.

Current live outcome:
- Claude web can authenticate to the private `Ecomlabs Tools` connector through Supabase OAuth.
- Claude can call the live shared reporting tool belt against Ecomlabs Tools data:
  - `resolve_client`
  - `list_wbr_profiles`
  - `get_wbr_summary`
  - `draft_wbr_email`
...
- A compact Claude Project bundle now lives in `docs/claude_project/` so the team rollout can use durable project instructions and narrow reference files instead of large planning docs.
...
- ClickUp task operations are expected to move to Claude's native ClickUp connector for the team rollout instead of staying in the Ecomlabs Tools MCP surface.

This does **not** replace The Claw in Slack. The current surface split is:
- **The Claw in Slack** for quick operational requests.
- **Claude.ai + Ecomlabs Tools integration** for higher-capability WBR and analyst-style workflows.
